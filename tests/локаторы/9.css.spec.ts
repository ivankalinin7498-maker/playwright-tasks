...
test.describe('Динамический контент с условиями', () => {
  ...
  test('Фильтрация динамических элементов', async ({ page }) => {
    ...
    const dynamicButton = page.locator('#dynamic-content .btn.disabled:has-text("Недостучно"):not([type="submit"])');
  ...
  test('Комбинации с :has и :not', async ({ page }) => {
    ...
    const availableProducts = page.locator('.product-card:not(.sold-out):has(.btn:has-text("В корзину"))');
    ...
    const activeUserCells = page.locator('#user-table tr:has(.status-active) td:not(:nth-child(3))'); // TODO(student): замените на корректный локатор
